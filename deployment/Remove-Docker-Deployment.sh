...
readarray -t CONTAINERS_TO_REMOVE="$(docker container ls --filter name="TheMover*" -q)"
...
readarray -t IMAGES_TO_REMOVE="$(docker images --filter reference="*/*/the-mover*" -q)"
...
readarray VOLUMES_TO_REMOVE="$(docker volume ls --filter name="TheMover*" -q)"
...
readarray NETWORKS_TO_REMOVE="$(docker network ls --filter name="TheMover*" -q)"
